...
def parse_datetime(date_str, time_str):
    ...
    for fmt in ('%d/%m/%Y %I:%M %p','%d/%m/%Y %H:%M'):
        try:
            dt = datetime.strptime(f'{date_str} {t}', fmt)
            return dt.replace(tzinfo=SV_TZ)  # Tag as El Salvador UTC-6
        except: pass
    return None
...
# ── Blocklist — grupos personales / no-clientes ───────────────────────────────
# Nombres normalizados (lowercase, sin emojis) que deben ignorarse SIEMPRE
BLOCKED_NAMES = {
    "niña maitra's beer&grill",
    "nina maitra's beer&grill",
    "familia 2.0",
    "2nd grade a",
    "5th b",
    "jimy",                        # grupo personal
...
def is_blocked(filename: str) -> bool:
    """Return True if this chat file should never be synced."""
    name = project_name(filename).lower().strip()
...
def sync_file(sb: Client, filepath: str, filename: str):
    ...
    # Skip blocklisted groups permanently
    if is_blocked(filename):
        log.info(f"  ⛔ Blocked group — skipping.")
        return
